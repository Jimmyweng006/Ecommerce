...
if [[ $# -eq 0 ]]; then
  ...
  echo "Example: CONTAINERS=\"ecommerce-app ecommerce-db\" OUT=docker-stats.log ./scripts/loadtest_with_stats.sh k6 run load-test/products-browse.js"
  exit 1
fi

CONTAINERS=${CONTAINERS:-"ecommerce-app ecommerce-db"}
INTERVAL=${INTERVAL:-1}
